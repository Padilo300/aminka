...
class ControllerCheckoutOctFastorder extends Controller {
    public function index() {
        ...
        $this->load->language('checkout/oct_fastorder');
        $this->document->setTitle($this->language->get('heading_title'));
        
        $data['heading_title']                  = $this->language->get('heading_title');
        $data['button_checkout']                = $this->language->get('button_checkout');
        $data['text_address_existing_payment']  = $this->language->get('text_address_existing_payment');
        $data['text_address_existing_shipping'] = $this->language->get('text_address_existing_shipping');
        ...
        $data['text_option_sku']                = $this->language->get('text_option_sku');
        $data['text_option_model']              = $this->language->get('text_option_model');
        $data['text_shipping_arderss']          = $this->language->get('text_shipping_arderss');
    ...
    public function shipping_method($render = true, &$data = array()) {
        ...
        if (isset($country_id)) {
            ...
            $session_guest_shipping = isset($this->session->data['guest']['shipping']) ? $this->session->data['guest']['shipping'] : array();
    ...
    public function payment_method($render = true, &$data = array()) {
        ...
        if (isset($this->request->post['country_id'])) {
            ...
            $this->session->data['guest']['payment']['payment_country_id'] = $this->request->post['country_id'];
    ...
    public function guest_validate() {
        ...
        if (!$json) {
            ...
            if (isset($this->request->post['telephone']) && ((utf8_strlen(str_replace(array(
                '_',
                '-',
                '(',
                ')',
        ...
        if (!$json) {
            ...
            if ($this->config->get('config_account_id')) {
                ...
                if ($information_info && !isset($this->request->post['payment_agree'])) {
                    $json['error']['warning_payment_agree'] = sprintf($this->language->get('error_agree'), $information_info['title']);
                }
            }
    ...
    public function status_cart() {
        ...
        $json['text_items'] = sprintf($this->language->get('text_items'), $this->currency->format($total, $this->session->data['currency']), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0));
