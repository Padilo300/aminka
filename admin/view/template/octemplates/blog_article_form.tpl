        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-article" class="form-horizontal">
          ...
          <div class="tab-content">
              ...
              <div class="tab-content">
                ...
                <div class="tab-pane" id="language<?php echo $language['language_id']; ?>">
                  <div class="form-group required">
                    ...
                    <div class="col-sm-10">
                      <input type="text" name="article_description[<?php echo $language['language_id']; ?>][name]" value="<?php echo isset($article_description[$language['language_id']]) ? $article_description[$language['language_id']]['name'] : ''; ?>" placeholder="<?php echo $entry_name; ?>" id="input-name<?php echo $language['language_id']; ?>" class="form-control seotranslit" />
            ...
            <div class="tab-pane" id="tab-links">
              <div class="form-group">
                <label class="col-sm-2 control-label" ><?php echo $entry_main_category_id; ?></label>
                <div class="col-sm-10">
                  <select name="main_oct_blog_category_id" class="form-control">
                    <option value="0" selected="selected"><?php echo $text_none; ?></option>
                    <?php foreach ($blog_categories as $category) { ?>
                    <?php if ($category['oct_blog_category_id'] == $main_oct_blog_category_id) { ?>
                    <option value="<?php echo $category['oct_blog_category_id']; ?>" selected="selected"><?php echo $category['name']; ?></option>
                    <?php } else { ?>
                    <option value="<?php echo $category['oct_blog_category_id']; ?>"><?php echo $category['name']; ?></option>
                    <?php } ?>
                    <?php } ?>
                  </select>
                </div>
              ...
              <div class="form-group">
                ...
                <div class="col-sm-10">
                  <div class="well well-sm" style="min-height: 150px;max-height: 500px;overflow: auto;">
                    ...
                    <?php foreach ($blog_categories as $article_category) { ?>
                    <tr>
                      <td class="checkbox">
                        <label>
                          <?php if (in_array($article_category['oct_blog_category_id'], $article_categories)) { ?>
                          <input type="checkbox" name="article_category[]" value="<?php echo $article_category['oct_blog_category_id']; ?>" checked="checked" />
                          <?php echo $article_category['name']; ?>
                          <?php } else { ?>
                          <input type="checkbox" name="article_category[]" value="<?php echo $article_category['oct_blog_category_id']; ?>" />
                          <?php echo $article_category['name']; ?>
                          <?php } ?>
                        </label>
                      </td>
              ...
              <div class="form-group">
                <label class="col-sm-2 control-label" for="input-related-p"><span data-toggle="tooltip" title="<?php echo $help_related; ?>"><?php echo $entry_related_p; ?></span></label>
                <div class="col-sm-10">
                  <input type="text" name="related_p" value="" placeholder="<?php echo $entry_related_p; ?>" id="input-related-p" class="form-control" />
                  <div id="product-related" class="well well-sm" style="height: 150px; overflow: auto;">
                    <?php $rp_row = 0; ?>
                    <?php foreach ($product_relateds as $product_related) { ?>
                    <div id="product-related<?php echo $rp_row; ?>"><i class="fa fa-minus-circle"></i> <?php echo $product_related['name']; ?>
              ...
              <div class="form-group">
                <label class="col-sm-2 control-label" for="input-related-a"><span data-toggle="tooltip" title="<?php echo $help_related; ?>"><?php echo $entry_related_a; ?></span></label>
                <div class="col-sm-10">
                  <input type="text" name="related_a" value="" placeholder="<?php echo $entry_related_a; ?>" id="input-related-a" class="form-control" />
                  <div id="article-related" class="well well-sm" style="height: 150px; overflow: auto;">
                    ...
                    <div id="article-related<?php echo $ra_row; ?>"><i class="fa fa-minus-circle"></i> <?php echo $article_related['name']; ?>
                      <input type="hidden" name="article_related[]" value="<?php echo $article_related['oct_blog_article_id']; ?>" />
                    </div>
                    <?php $ra_row++; ?>
                    <?php } ?>
                  </div>
...
$('input[name=\'blog_category\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=octemplates/blog_category/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
						label: item['name'],
						value: item['oct_blog_category_id']
					}
				}));
			}
		});
...
// Related Article
$('input[name=\'related_a\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=octemplates/blog_article/autocompleteArticle&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
						label: item['name'],
						value: item['oct_blog_article_id']
					}
				}));
			}
		});
...
// Related Product
$('input[name=\'related_p\']').autocomplete({
  'source': function(request, response) {
    $.ajax({
      url: 'index.php?route=octemplates/blog_article/autocompleteProduct&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
      dataType: 'json',
      success: function(json) {
        response($.map(json, function(item) {
          return {
