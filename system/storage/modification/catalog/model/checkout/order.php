...
class ModelCheckoutOrder extends Model {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				foreach ($product['option'] as $option) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "order_option SET order_id = '" . (int)$order_id . "', order_product_id = '" . (int)$order_product_id . "', product_option_id = '" . (int)$option['product_option_id'] . "', product_option_value_id = '" . (int)$option['product_option_value_id'] . "', name = '" . $this->db->escape($option['name']) . "', `value` = '" . $this->db->escape($option['value']) . "', `sku` = '" . $this->db->escape(isset($option['sku']) ? $option['sku'] : '') . "', `model` = '" . $this->db->escape(isset($option['model']) ? $option['model'] : '') . "', oct_quantity_value = '" . (int)((isset($option['oct_quantity_value'])) ? $option['oct_quantity_value'] : 0) . "', `type` = '" . $this->db->escape($option['type']) . "'");
				}
			}
		}
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
        ...
        $data['oct_text_option_sku'] = $language->get('oct_text_option_sku');
        $data['oct_text_option_model'] = $language->get('oct_text_option_model');
